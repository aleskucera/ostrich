"""Helhest Junior spline trajectory optimization over the surface hill.

Port of examples/helhest/gradient/trajectory_spline_surface_fast.py to the
helhest_junior robot. Identical optimization (spline-parameterized wheel
velocities, Adam, trajectory + yaw + regularization loss); the only change is
the robot model (create_helhest_junior_model) and its gains/frictions/masses.

The junior shares the same DOF layout as helhest:
  [0..5] = free base joint, [6] = left wheel, [7] = right wheel, [8] = rear wheel
so WHEEL_DOF_OFFSET / NUM_WHEEL_DOFS are unchanged.
"""
import argparse
import os
import pathlib
...
class HelhestJuniorTrajectorySplineSurfaceOptimizer(AxionDifferentiableSimulator):
    ...
    def build_model(self) -> Model:
        ...
        # Joint control gains — use the junior's tuned motor gain.
        TARGET_KE = HelhestJuniorConfig.TARGET_KE  # 150.0
        TARGET_KD = 0.0
...
def main():
    ...
    sim = HelhestJuniorTrajectorySplineSurfaceOptimizer(
        sim_config,
        render_config,
        engine_config,
        logging_config,
