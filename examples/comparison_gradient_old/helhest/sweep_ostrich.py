"""Hyperparameter sweep for Ostrich helhest — minimize trajectory error vs ground truth.

Each config runs in a subprocess to avoid GPU memory leaks between runs.

For Ostrich the sweepable parameters are:
  - k_p (velocity servo proportional gain, = target_ke)
  - friction_left_right (front wheel friction)
  - friction_rear (rear wheel friction)
  (dt is fixed at 0.05 — Ostrich's implicit solver handles large timesteps)

Usage:
    python examples/comparison_gradient/helhest/sweep_ostrich.py \
...
def run_single_config(dt, k_p, friction_lr, friction_rear):
    """Run one Ostrich config in a subprocess, return trajectory as list of [x, y]."""
...
class SweepSim(OstrichDifferentiableSimulator):
