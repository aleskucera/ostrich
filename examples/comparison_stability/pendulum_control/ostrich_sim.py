"""Control stability benchmark — Ostrich (implicit servo).

Scene: single pendulum, pivot fixed at (0,0,2), 1m link, 1 kg.
       Displaced to θ₀ = π/3 rad at t=0. Target: θ = 0 (hanging at rest).

Uses Ostrich's implicit servo constraint (JointMode.TARGET_POSITION, ke=kp, kd=kd).
...
Usage:
    python examples/comparison/control_stability/ostrich.py \
        --experiment dt_sweep \
        --save examples/comparison/control_stability/results/ostrich_implicit_dt.json

    python examples/comparison/control_stability/ostrich.py \
        --experiment binary_search \
        --save examples/comparison/control_stability/results/ostrich_threshold.json
"""
import argparse
import json
import math
...
# --- Scene constants (ostrich-specific geometry) ---
...
class PendulumSim(InteractiveSimulator):
    """Single-pendulum stability simulator using Ostrich implicit servo."""
...
# Factory helpers
# ---------------------------------------------------------------------------
def _make_engine_config(kp: float, kd: float) -> OstrichEngineConfig:
...
def main():
    ...
    if args.experiment == "dt_sweep":
        ...
        print(f"Ostrich-Implicit — dt_sweep (kp={kp}, kd={kd}):")
        for dt in DT_VALUES:
        ...
        dt = GAIN_SWEEP_DT
        print(f"Ostrich-Implicit — gain_sweep (dt={dt}s):")
        for kp in KP_VALUES:
            kd = kd_from_kp(kp)
        ...
        print(f"Ostrich-Implicit — binary_search (kp={kp}, kd={kd}):")
