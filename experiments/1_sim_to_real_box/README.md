...
2. **`sweep_ostrich.py`** — reuses `HelhestJuniorReplaySimulator` (junior model
...
python experiments/1_sim_to_real_box/sweep_ostrich.py \
...
|---|---|---|---|---|
| **Ostrich** | **0.062 m** | 3.4° | `mu_rear=1.5, compliance.contact=1e-7` | 0.05 |
...
**Ostrich is the most accurate**, edging out MuJoCo by ~3 mm despite a
...
> of dt — adequate for Ostrich/MuJoCo's implicit contact (converges in a few
...
|---|---|---|
| initial (mu=0.5, ke=4e4) | 1.5 m | wheels barely spun — `k_d=0` ok for Ostrich, broken for SI |
...
1. **`k_d=0` makes the wheels not spin** (in `TARGET_VELOCITY` mode, SI consumes
   `target_kd` as the velocity-feedback gain; Ostrich doesn't). One forgotten
...
3. **The stable parameter region is narrow** — many neighbouring configs
   diverge (combined of 14, 18, 28, 100 m, or NaN). MuJoCo and Ostrich don't
...
¹ NB: `ShapeConfig.ke/kd/kf` are not consumed by the Ostrich solver (Ostrich uses
its own compliance model); the `--ke` CLI knob is kept for signature parity
but has no effect.

**Notable asymmetry:** Ostrich's `mu_rolling` — the natural analog of MuJoCo's
torsional friction — is *flat* across 0–5.0 (Δ ≈ 4 mm). Ostrich's plain friction
