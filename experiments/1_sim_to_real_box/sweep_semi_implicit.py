...
def run_config(dt, mu, ke, kd, kf, k_d_act, gts, use_graph):
...
def main():
    ...
    # Defaults are this box scene's tuned best (combined = 0.204 m at floor).
    # Notes:
    #  - mu is ~5-10x smaller than physical rubber: SemiImplicit's spring-damper
    #    friction model lets the same effective force come from kf*slip*mu, so
    #    "physical" mu would overshoot.
    #  - ke had to be ~5x exp-1's helhest value (8000) because the junior chassis
    #    is heavier (90 kg) and we need a stiffer contact to keep it from
    #    sinking into the box during the climb.
    ...
    ap.add_argument("--mu", type=float, nargs="+", default=[0.08, 0.1, 0.15])
    ap.add_argument("--ke", type=float, nargs="+", default=[6e4, 8e4, 1.2e5])
    ap.add_argument("--kd", type=float, nargs="+", default=[2e3])
    ap.add_argument("--kf", type=float, nargs="+", default=[1500.0])
    ap.add_argument("--k-d-act", type=float, nargs="+", default=[200.0, 250.0],
    ...
    configs = list(itertools.product(args.dt, args.mu, args.ke, args.kd, args.kf, args.k_d_act))
    print(f"Semi-Implicit box sweep: {len(configs)} configs x {len(gts)} runs "
          f"({'cuda-graph' if use_graph else 'python-loop'})")

    best, rows = None, []
    for dt, mu, ke, kd, kf, k_d_act in configs:
        t0 = time.perf_counter()
        try:
            scores = run_config(dt, mu, ke, kd, kf, k_d_act, gts, use_graph)
