"""Ostrich parameter sweep for the box sim-to-real benchmark.
...
Usage:
    python experiments/1_sim_to_real_box/sweep_ostrich.py \
        ...
        --save results/sweep_ostrich.json
"""
import argparse
import itertools
import json
...
def main():
    ...
    # NOTE: ShapeConfig.ke/kd/kf are NOT consumed by the Ostrich solver — Ostrich
    # uses its own compliance model. Kept here only for parity with the
    # constructor signature; sweeping these does nothing for Ostrich accuracy.
    ap.add_argument("--ke", type=float, nargs="+", default=[150],
                    help="(IGNORED by Ostrich solver; kept for signature parity)")
    ...
    ap.add_argument("--save", default=str(RESULTS_DIR / "sweep_ostrich.json"))
    args = ap.parse_args()
    ...
    print(f"Ostrich box sweep: {len(configs)} configs x {len(gts)} runs "
