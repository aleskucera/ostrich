"""Interactive Ostrich obstacle traversal — for visual tuning before dt sweep.
...
Usage:
    python examples/comparison_gradient/helhest/dt_sweep/tune_obstacle_ostrich.py
    python examples/comparison_gradient/helhest/dt_sweep/tune_obstacle_ostrich.py --dt 0.05
    python examples/comparison_gradient/helhest/dt_sweep/tune_obstacle_ostrich.py --obstacle-height 0.15
"""
import argparse
import os
...
class HelhestObstacleSim(InteractiveSimulator):
    ...
    def set_friction_coefficient(self, mu: float, obstacle_mu: float = 1.0):
        ...
        wp.launch(
            ...
            dim=(self.solver.dims.num_worlds, self.solver.ostrich_model.shape_count),
            inputs=[mu],
            outputs=[self.solver.ostrich_model.shape_material_mu],
        )
        # Override obstacle box with higher friction
        # The box is the last shape added in build_model
        obstacle_idx = self.solver.ostrich_model.shape_count - 1
        wp.launch(
