...
PY_AXION="${PY_AXION:-$HOME/projects/axion/.venv/bin/python}"        # axion + Newton/Warp (Ostrich, Semi, XPBD)
PY_BRAX="${PY_BRAX:-$HOME/projects/axion/.venv-brax/bin/python}"     # brax + jax
PY_MJX="${PY_MJX:-$HOME/projects/axion/.venv-mjx/bin/python}"        # mujoco-mjx + jax  (set to your MJX venv)
...
run ostrich        "$PY_AXION" "$HERE/optimize_axion.py"
# MJX (reference; works throughout)
run mjx            "$PY_MJX"   "$HERE/optimize_mjx.py"
# Newton/Warp solvers
run semi_implicit  "$PY_AXION" "$HERE/optimize_semi_implicit.py"
run xpbd           "$PY_AXION" "$HERE/optimize_xpbd.py"
