"""Helhest_junior box trajectory optimization using Newton XPBD
+ Warp tape BPTT.

XPBD counterpart of optimize_semi_implicit.py. Identical model, loss, spline,
and Adam optimizer; the only change is Newton's solver (SolverXPBD instead of
SolverSemiImplicit), so the two scripts isolate the effect of the solver on
gradient quality. Optimises a K-knot wheel-velocity spline so the
helhest_junior matches a recorded real trajectory while crossing the box, with
gradients via backpropagation through Warp's computation tape over the XPBD
...
# ------------------------------ optimizer ------------------------------------
class HelhestJuniorBoxXPBDOptimizer(NewtonDifferentiableSimulator):
...
def make_configs(duration, dt):
    ...
    rc = RenderingConfig(vis_type="null", target_fps=max(1, int(1 / dt)),
                          start_paused=False)
    ec = XPBDEngineConfig(iterations=2)
...
def run_trial(target_xyz_rel, target_t, K, lr, iterations, seed, duration, dt,
    ...
    sim = HelhestJuniorBoxXPBDOptimizer(sim_cfg, rc, ec, LoggingConfig(),
...
def main():
    ...
    print(f"-> {int(args.horizon_s / args.dt)} XPBD steps per iter "
    ...
    save_path = args.save or str(RESULTS_DIR / "xpbd.json")
