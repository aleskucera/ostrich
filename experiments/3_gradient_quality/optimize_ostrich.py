"""Helhest trajectory optimization using Ostrich adjoint gradients.

Optimizes K spline control points to match a real robot trajectory.
Uses calibrated physics params from Experiment 1.

Usage:
    python experiments/3_gradient_quality/optimize_ostrich.py \
        --ground-truth ../data/right_turn_b.json

    python experiments/3_gradient_quality/optimize_ostrich.py \
        --ground-truth ../data/right_turn_b.json \
        --save results/ostrich.json --iterations 200 --lr 0.1
"""
import argparse
import json
import os
...
# Calibrated params from Experiment 1 (sweep_ostrich.json), using
...
class HelhestOptimizer(OstrichDifferentiableSimulator):
    def __init__(
        self,
        sim_config,
        render_config,
...
def main():
    ...
    parser.add_argument("--save", metavar="PATH", default=str(RESULTS_DIR / "ostrich.json"))
