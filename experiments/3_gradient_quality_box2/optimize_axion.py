"""Helhest_junior box random-IC final-pose optimisation — Axion.
...
Outputs results/axion[_<save>].json with per-trial loss curves AND derived
metrics (final-pose error, terminal speed, control jerk, success flag).

Usage:
    python experiments/3_gradient_quality_box2/optimize_axion.py
"""
import argparse
import json
import os
...
from axion.collision.config import ContactReductionConfig
...
# ------------------------------ optimizer ------------------------------------
class HelhestJuniorBoxFinalPoseOptimizer(AxionDifferentiableSimulator):
...
def make_configs(duration, dt):
    ...
    ec = AxionEngineConfig(
        ...
        # examples/conf/engine/axion.yaml). Collapses redundant wheel-on-box
...
def check_gradient_finite_difference(seed, K, horizon, dt, ic, target, weights,
    ...
    """Compare Axion's autodiff gradient ∇_θ L to the central finite-difference
...
def main():
    ...
    ap.add_argument("--check-gradient", action="store_true",
                    ...
                    "per-parameter so you can see if Axion's adjoint is "
