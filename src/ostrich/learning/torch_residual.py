...
from ostrich.optim import FullSystemOperator

from ostrich.core.contacts import OstrichContacts
from ostrich.core.linear_system import compute_linear_system


class OstrichResidual(torch.autograd.Function):
    @staticmethod
    def forward(
        ctx,
        model: OstrichModel,
        ...
        ctx.ostrich_args = (model, contacts, data, config, dims, full_A_op)
    ...
    def backward(ctx, grad_output):
        model, contacts, data, config, dims, full_A_op = ctx.ostrich_args
...
class OstrichResidualLoss(nn.Module):
    def __init__(
        self,
        model: OstrichModel,
        contacts: OstrichContacts,
