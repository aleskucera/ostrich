...
The backward uses wp.Tape through the residual computation (like OstrichResidualAD)
...
from ostrich.core.linear_system import (
    compute_linear_system,
    compute_dbody_qd_from_dbody_lambda,
)
from ostrich.core.model import OstrichModel
from ostrich.mechanics import integrate_body_pose_kernel
from ostrich.optim import PCRSolver, SystemOperator, JacobiPreconditioner
...
class DifferentiableNewtonStep(torch.autograd.Function):
    ...
    def forward(
        ...
        ctx.ostrich_args = (model, contacts, data, config, dims, pcr_solver, A_op, preconditioner)
    ...
    def backward(ctx, grad_vel_new, grad_cf_new):
        ...
        """
        model, contacts, data, config, dims, pcr_solver, A_op, preconditioner = ctx.ostrich_args
