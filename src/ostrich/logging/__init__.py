"""Persistent-state logging for the Ostrich engine.

Three domain-specific HDF5 loggers, each consumed by OstrichEngineBase
when the corresponding sub-config (``LoggingConfig.{hdf5,dataset,adjoint}``)
is enabled. The configs themselves live in ``ostrich.core.logging_config``
because they are part of the engine config tree.

``HDF5Reader`` is the read side, used by the dt-stability diagnostics to
inspect what the loggers wrote.
"""
from .adjoint_logger import AdjointHDF5Logger
from .dataset_logger import DatasetHDF5Logger
from .hdf5_reader import HDF5Reader
from .sim_logger import SimulationHDF5Logger


__all__ = [
    "AdjointHDF5Logger",
    "DatasetHDF5Logger",
    "HDF5Reader",
    "SimulationHDF5Logger",
]
