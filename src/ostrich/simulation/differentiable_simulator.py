...
class OstrichDifferentiableSimulator(DifferentiableSimulator, ABC):
    """
    Differentiable simulator using the Ostrich implicit integration engine.

    Uses implicit differentiation via a TrajectoryBuffer and a custom
    backward pass through the Ostrich solver.
    """
    ...
    def __init__(
        ...
        self.trajectory = TrajectoryBuffer(
            ...
            contacts=self.solver.ostrich_contacts,
    ...
    def _forward_backward(self):
        ...
        for i in range(self.clock.total_sim_steps - 1, -1, -1):
            self.trajectory.load_step(i, self.solver.data, self.solver.ostrich_contacts)
