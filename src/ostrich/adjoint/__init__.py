"""Adjoint (reverse-mode autodiff) kernels for the Ostrich engine.
...
Consumed by ``OstrichEngineBase`` when the engine is constructed with
``differentiable_simulation=True``. The ``AdjointConfig`` sub-config
itself stays in ``ostrich.core.engine_config`` because it is part of the
engine config tree.
"""
