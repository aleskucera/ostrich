...
This subpackage sits between Newton's narrow-phase output and Ostrich's
...
def build_reducer(
    ...
    Args:
        ...
        ostrich_model: Source of ``shape_body`` (per-world shape→body table).
    ...
    if policy == "top_k":
        ...
        return TopKReducer(cfg, ostrich_model, data, dims, device)
    if policy == "fps":
        from .fps import FPSReducer

        return FPSReducer(cfg, ostrich_model, data, dims, device)
    if policy == "cluster":
        from .cluster import ClusterReducer

        return ClusterReducer(cfg, ostrich_model, data, dims, device)
