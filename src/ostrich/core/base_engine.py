...
import numpy as np
import warp as wp
from ostrich.collision import build_reducer
...
from ostrich.optim import SystemLinearData
...
from ostrich.logging import SimulationHDF5Logger
from ostrich.profiling import EngineProfiler

from .backtracking import perform_backtracking
from .contacts import OstrichContacts
...
from .model import OstrichModel
...
class OstrichEngineBase(SolverBase):
    def __init__(
        self,
        model: Model,
        sim_steps: int,
        config: Optional[OstrichEngineConfig] = OstrichEngineConfig(),
        ...
        # --- 2. Model & Data Setup ---
        self.ostrich_model = OstrichModel(model)
        self.ostrich_contacts = OstrichContacts(model, self.config.contacts.max_per_world)

        self.dims = EngineDimensions(
            num_worlds=self.ostrich_model.num_worlds,
            body_count=self.ostrich_model.body_count,
            contact_count=self.ostrich_contacts.max_contacts,
            joint_count=self.ostrich_model.joint_count,
            joint_dof_count=self.ostrich_model.joint_dof_count,
            linesearch_step_count=self.config.linesearch.num_steps,
            joint_constraint_count=self.ostrich_model.num_joint_constraints,
            control_constraint_count=self.ostrich_model.num_control_constraints,
        )

        self.data = EngineData(
            model=self.ostrich_model,
            dims=self.dims,
            config=self.config,
            device=self.device,
        ...
        self.warm_starter = ContactWarmStarter(
            ...
            ostrich_model=self.ostrich_model,
            data=self.data,
            dims=self.dims,
            device=self.device,
        ...
        if self.logging_config.dataset.enabled:
            self.dataset_logger = DatasetHDF5Logger(
                ...
                model=self.ostrich_model,
                data=self.data,
                contacts=self.ostrich_contacts,
                config=self.config,
                dims=self.dims,
                device=self.device,
            )
    ...
    def load_data(
        ...
        self.ostrich_contacts.load_contact_data(
            contacts,
            self.ostrich_model,
            self.data,
            self.dims,
        ...
        self.contact_reducer.apply(self.ostrich_contacts)
        ...
        self.warm_starter.apply(self.ostrich_contacts, self.data, dt)
    ...
    def compute_warm_start_forces(self):
        ...
        wp.launch(
            ...
            inputs=[
                ...
                self.ostrich_model.shape_material_mu,
                self.ostrich_model.shape_friction_axis_local,
                self.ostrich_model.shape_mu_perp,
                self.ostrich_model.shape_body,
                self.data.body_pose_prev,
                self.ostrich_contacts.contact_count,
                self.ostrich_contacts.contact_shape0,
                self.ostrich_contacts.contact_shape1,
                self.ostrich_contacts.contact_normal,
            ],
            device=self.device,
        )
    ...
    def _solve(self):
        ...
        def nr_loop_step(slot_idx: int = 0):
            ...
            compute_dbody_qd_from_dbody_lambda(self.ostrich_model, self.data, self.config, self.dims)
            ...
            if self.config.linesearch.enabled:
                ...
                apply_standard_newton_step(self.ostrich_model, self.data, self.dims)
                compute_residual(
                    self.ostrich_model, self.ostrich_contacts, self.data, self.config, self.dims
                )
        ...
        perform_backtracking(self.ostrich_model, self.data, self.config, self.dims)

        # Snapshot the post-backtrack converged state so the next step
        # can warm-start its contacts. No-op when warm start is
        # disabled.
        self.warm_starter.snapshot(self.ostrich_contacts, self.data)
    ...
    def step_backward(self):
        from ostrich.adjoint import adjoint_regularize_compliance_kernel
        from ostrich.adjoint import freeze_contact_mode_kernel
        from ostrich.adjoint import freeze_contact_mode_soft_kernel
        ...
        wp.launch(
            ...
            inputs=[
                ...
                self.data.body_pose,
                self.ostrich_model.body_com,
                self.ostrich_model.body_inv_mass,
                self.ostrich_model.body_inv_inertia,
                self.data.dt,
            ],
            outputs=[
